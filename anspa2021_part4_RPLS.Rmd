...
```{r, eval=FALSE}
 library(data.table)
rpls <- fread("data2021/93/geoloc2020_detail_IDF_dep_93.csv",
              encoding = "UTF-8",
              dec=",")

rpls<-as.data.frame(rpls)

saveRDS(rpls,"data2021/93/RPLS2020.RDS")
```
...
- result_label : adresse
- CODECOM : nom de la commune
- DEPCOM : code insee de la commune
- PLG_IRIS : code de l'IRIS dans la commune
...
On produit un résumé rapide des variables retenues :
...
```{r}
saveRDS(sel,"data2021/94/RPLS2020_sel.RDS")
```
